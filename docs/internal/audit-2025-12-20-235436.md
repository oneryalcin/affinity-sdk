...
- **TC-002–TC-005**: **Implemented** — async tests added for v1-only services
...
- **ERR-007**: **Implemented** — list service response parsing now wraps
  ...
  (`affinity/services/lists.py`).
- **DEP-001/DEP-002**: **Implemented** — `pyproject.toml` now caps httpx/pydantic upper bounds.
- **DOC-005**: **Implemented** — `docs/public/guides/sync-vs-async.md` parity table now includes rate_limits.

### Low
- **API-013**: **Implemented** — sync EntityFileService.download now supports
  `deadline_seconds` (`affinity/services/v1_only.py`, `affinity/clients/http.py`).
- **TC-006–TC-010**: **Implemented** — async tests added for field/field_values/
  relationship_strength/rate_limit/auth services.
...
- **SEC-002**: **Implemented** — webhook sent_at validation added with replay window
  checks (`affinity/inbound_webhooks.py`).
- **ERR-004**: **Implemented** — added rate-limit gating to avoid herd retries
  (`affinity/clients/http.py`).
- **DEP-003**: **Implemented** — `rich` dependency now capped with an upper bound.
...
Async service tests cover list/get/create/update/delete flows for notes, reminders,
webhooks, and interactions.

**Evidence:** `tests/test_services_v1_only_async.py`
...
`httpx` now capped with an upper bound (`<1.0.0`).
...
`pydantic` now capped with an upper bound (`<3.0.0`).
...
Sync EntityFileService.download now accepts `deadline_seconds` and forwards it to
HTTPClient.download_file, matching async behavior.
...
Async service tests cover field/field-values, relationship-strength, rate-limit,
and auth surfaces.
...
Webhook parsing now validates `sent_at` against a replay window and future skew
to prevent stale or suspicious payloads.
...
Requests now respect a shared rate-limit gate before sending, and 429 responses
set the gate to stagger subsequent retries.
...
`rich` now capped with an upper bound (`<14`).
