...
- **API-007**: **Implemented** — `AsyncListEntryService` now exposes write ops and field value helpers
  (`affinity/services/lists.py`).
- **CLI-002/003/004/005**: **Valid** — no CLI modules for notes/reminders/webhooks/interactions
...
- **CLI-006–CLI-014**: **Valid** — CLI only exposes search/get/ls/view; no create/update/delete/
  merge/field/field_values/relationship_strength/task commands.
- **ERR-007**: **Valid** — `model_validate()` calls in list services are not wrapped
  (`affinity/services/lists.py:184`).
...
- PersonService.create/update/delete not exposed
- CompanyService.create/update/delete not exposed
- ListService.create not exposed
- ListEntryService write operations not exposed
- PersonService.merge/get_merge_status not exposed
- CompanyService.merge/get_merge_status not exposed
- FieldService and FieldValueService have no CLI commands
- RelationshipStrengthService has no CLI commands
- TaskService has no CLI commands
...
Service methods call model_validate() on API responses without try/except handling. Raw pydantic.ValidationError may propagate to users.
