...
class TestCompanySearch:
    """Tests for company search command."""

    def test_search_basic(self, respx_mock: respx.MockRouter) -> None:
        """Basic company search should work."""
    ...
    def test_search_with_page_size(self, respx_mock: respx.MockRouter) -> None:
        ...
        result = runner.invoke(
            cli,
            ["--json", "company", "search", "acme", "--page-size", "10"],
            env={"AFFINITY_API_KEY": "test-key"},
        )
        assert result.exit_code == 0

    def test_search_with_all_pages(self, respx_mock: respx.MockRouter) -> None:
        """Company search with --all flag."""
        respx_mock.get("https://api.affinity.co/organizations").mock(
            side_effect=[
                Response(
                    200,
